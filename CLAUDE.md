...
- **Landing Page & UI Polish (2026-03-27) — Done**
  - Landing pages (`/` and `/landing`): removed "Built with Claude Code", added Marketplace Connector + Conversational Memory features, renamed Deep Research → Analytical Autopilot, updated stats to 567+
  - New landing sections: Integration logos strip, Comparison table (vs ChatGPT/RAG/BI), Demo video placeholder, Pricing teaser (3-tier)
  - Improved hero badge, sub-headline, features heading, CTA copy
  - Chat app font normalization: `text-[9px]`→`text-[10px]`, `text-[11px]`→`text-xs`, `text-[15px]`→`text-sm`
