...
async def generate_analysis_code(
    ...
    system_prompt = (
        ...
        "A DataFrame named `df` is ALREADY bound to the dataset shown below.\n"
        "You may use pd, np, plt, sns, json, df directly — do NOT re-import or rebind them.\n\n"
        "Given a question and the pre-loaded `df`, write Python code that:\n"
        "1. Performs relevant analysis on `df` to answer the question\n"
        f"2. CHART TYPE REQUIREMENT: {chart_instructions}\n"
        "3. Calls plt.show() exactly once after creating the chart\n"
        "4. Assigns a brief text summary to a variable named `result`\n"
        "5. AFTER plt.show(), prints chart metadata in EXACTLY this format (one line):\n"
