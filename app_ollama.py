...
# Load environment variables (though mostly not needed for purely local Ollama, good practice)
load_dotenv()

# --- CONSTANTS & CONFIG ---
DEFAULT_CHAT_MODEL = "llama3.2"
DEFAULT_VISION_MODEL = "llava" # Common vision model for Ollama
DEFAULT_EMBED_MODEL = "llama3.2" # Or "nomic-embed-text" if user has it
...
chat_model_name = st.sidebar.text_input("Chat Model:", value=DEFAULT_CHAT_MODEL, help="Name of the Ollama model for chat (e.g., llama3.2, mistral)")
vision_model_name = st.sidebar.text_input("Vision Model:", value=DEFAULT_VISION_MODEL, help="Name of the Ollama model for image analysis (e.g., llava)")
embed_model_name = st.sidebar.text_input("Embedding Model:", value=DEFAULT_EMBED_MODEL, help="Name of the Ollama model for embeddings")

# Initialize clients
try:
    llm = ChatOllama(model=chat_model_name, temperature=0)
except Exception as e:
    st.error(f"Failed to initialize ChatOllama with model '{chat_model_name}'. Is Ollama running?")
...
def get_vectorstore_from_pdfs(_uploaded_files, _chat_model_name, _vision_model_name, _embed_model_name, _deep_visual_mode=False):
    if _uploaded_files:
            ...
            try:
                ...
                for page_num, page in enumerate(doc):
                    ...
                    for img_index, img in enumerate(images):
                        ...
                        try:
                            # 2024 Fix: Ollama python library expects image bytes directly
                            # We use rate limiting protection just in case, though local is faster
                            description = ""
                            try:
                                response = ollama.chat(
                                    model=_vision_model_name,
                                ...
                                # Fallback or just ignore image if vision model fails/missing
                                pass
        ...
        if all_splits:
            try:
                embeddings = OllamaEmbeddings(model=_embed_model_name)
                # Test embedding to fail early if model missing
                embeddings.embed_query("test")
                db = FAISS.from_documents(all_splits, embeddings)
                return db
            except Exception as e:
                st.error(f"Error creating embeddings with model '{_embed_model_name}'. Ensure you have pulled it via `ollama pull {_embed_model_name}`.")
                return None
    return None
...
def analyze_document_persona(db, llm):
    ...
    try:
        ...
        # Handle cases where response is AIMessage or string
...
if db is not None:
    ...
    selected_persona = st.sidebar.selectbox(
        ...
        index=list(EXPERT_PERSONAS.keys()).index(suggested),
    ...
    if st.sidebar.button("Clear Chat"):
        ...
        st.session_state.chat_history = []  # Clear langgraph/langchain history if separate
