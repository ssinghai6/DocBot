...
### What Shipped (branch feature/DOCBOT-1101-landing-page-ui-polish, merged to main)

**Landing Page & UI Polish (DOCBOT-1101):**
- `src/app/page.tsx` + `src/app/landing/page.tsx` — Full content overhaul (both files kept in sync):
  - Removed "Built with Claude Code" → "AI-Powered Document Intelligence"
  - Updated stats: 567+ Test Coverage, 8 Expert AI Personas, 6+ Data Sources, 7 SQL Pipeline Steps
  - Added Marketplace Connector and Conversational Memory as 7th/8th feature cards
  - Renamed "Deep Research" → "Analytical Autopilot" with Wand2 icon and "Auto-Triggers" badge
  - New sections: Integration logos strip (PostgreSQL, MySQL, Azure SQL, Amazon, LangGraph, E2B), Comparison table (vs ChatGPT/Custom RAG/BI Tools), Demo video placeholder, Pricing teaser (Starter/Pro/Enterprise)
  - Updated hero badge, sub-headline, features heading, CTA copy
- Chat app visual polish across 9 component files:
  - Font normalization: `text-[9px]`→`text-[10px]`, `text-[11px]`→`text-xs`, `text-[15px]`→`text-sm`
  - Placeholder color standardized to `placeholder-gray-500`
